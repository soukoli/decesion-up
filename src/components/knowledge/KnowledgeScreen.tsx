    ...
    <div className="h-full flex flex-col bg-slate-950">
      ...
      <div className="flex-shrink-0 px-4 pb-3 space-y-2">
        ...
        <div className="relative">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Hledat..." className="w-full pl-10 pr-4 py-2 bg-slate-800/30 border border-slate-700/50 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-500/50" />
        </div>
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain px-4 pb-6">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-400 text-sm">{search ? 'Žádné výsledky' : 'Žádné nápady'}</p>
          </div>
        ) : (
            ...
            {filtered.map(idea => {
              ...
              return (
                <div key={idea.id} className={`relative border rounded-xl transition-all overflow-hidden ${
                  ...
                  isDone ? 'bg-slate-800/10 border-slate-800/30' : 'bg-slate-800/30 border-slate-700/50'
                }`}>
                  ...
                  {isExpanded && (
                    <div className="px-3 pb-3 border-t border-slate-700/30">
                      {isEditing ? (
                        ...
                        <div className="mt-3">
                          {idea.context && (
                            <p className="text-sm text-slate-300 leading-relaxed mb-3">{idea.context}</p>
                          )}
                          {/* Actions */}
